...
async function createReview(req, res) {
    ...
    const acceptedOrder = await orderModel.exists({ user: req.user._id, food, status: { $in: ['preparing', 'out_for_delivery', 'delivered'] } });
    if (!acceptedOrder) return res.status(403).json({ message: 'The restaurant must accept your order before you can review it' });
